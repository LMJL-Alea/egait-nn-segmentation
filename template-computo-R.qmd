...
data.frame(
  ...
  values = c("1", "40, 60, 80", "Inverse class frequency weights: <br> c(1.89, 0.67, 1.90, 0.69)")
) |> 
  gt::gt(groupname_col = "strat") |> 
  gt::cols_label(
    param = "Parameter",
    values = "Tuning Grid"
  ) |> 
...
data.frame(
  ...
  values = c(
    ...
    "3, 5, 10, 20", "1, 2",
    "rep(100, 2), rep(500, 2), rep(100, 3), rep(500, 3), rep(100, 4), rep(500, 4)", "0.01, 0.001", "0.1, 0.3"
  )
) |> 
...
For both strategies and each model, we got the optimal parameters thanks to tuning (see @tbl-tuning-res-events and @tbl-tuning-res-phases in the appendix for the detail of chosen parameters for each model). We then evaluated each model on the test set which was never seen before to find the best model for each strategy (see @tbl-res).
...
data.frame(
  ...
  youden = c("67.6%", "60.0%", "70.3%", "70.5%", "83.0%", "24.0%", "86.5%"),
...
The neural network was selected to detect events and the boosted trees model was selected to detect phases. The next step is to compare the two strategies to choose a model between the two selected ones, to have a final model.


### Selecting a model

To choose a final model, we need to determine the best strategy so we compare the two best models with the real events given by the gold standard.

First, we shorten the times where we want to detect points by keeping only the time range where the gold standard detected the four events perfectly. Indeed, at the start and the end of the session, this tool can miss contact points because the subject is not walking on the mat sensors. At the start of the session, we search for the first "Heel Strike" event appearing in the session which is followed by the three other events in the correct order. At the end of the session, we remove all points after the first missing event.

When detecting events, the model does not detect only one point corresponding to the event but a window of points containing the event. This means that the predictions are similar to the ones given when detecting phases where the event is part of the predicted phase. We can use the same strategy for both strategy to choose one point either in the windows detected with the event strategy or in the phases predicted with the phase strategy

=> figure of predictions for each strategy?

In each window or phase detected, the following process is applied to keep one point representing the event. We first need to identify each of them correctly:

- a threshold to firstly select only the observations with a high probability of being in the class: $p > 0.4$.
- a time to know when to distinguish two predicted windows or phases: if the first point of the window $i$ is at more than $t = 40 ms$ from the last point of the window $i-1$, then we consider the windows distinct. This allows to have a window with points that are not necessarily consecutive even if only one event should be detected in this time range.

Then, in each of these clearly separated windows or phases, we keep the point having the highest probability of being in this class according to the model. This point is the estimated event.

We can then compare these estimated events with the ones given by the gold standard to see each of the strategy gives closer result.
