...
A wearable sensor was used to record the hip rotation. It contains an accelerometer, a gyroscope and a magnetometer. Subjects were wearing the sensor on their right hip (see @fig-sensor-position). The frequency of the sensor is 100Hz. With this device, the data acquired is in the form of unit quaternion time-series, representing the 3D rotation of the hip over time.
...
Six subjects have been included in this study (three men and three women) walking at different speeds to have a variety of gait data (see @tbl-subjects). We have in total 174 walking sessions.
...
Method 2: Predicting sub-phases

: The second method consists of predicting sub-phases in the signal that are characterized with the gait events. We label each part in the signal between two events as a sub-phase, leading to four classes for our observations:
...
In order to choose the best model for each strategy, we tuned the hyper-parameter of each model for both strategies. Since we do not use the same metric to tune and evaluate models if we detect events or sub-phases, we cannot directly choose a strategy at this step. Therefore, we select one model per strategy and compare the two strategies later.

Here are the tuning grid values used for the feature space parameter (@tbl-grid-fs), the class weights (@tbl-grid-class-weights) and the model hyper-parameters (@tbl-grid-model).
...
tibble::tibble(
  strat = c("Event Detection", "Event Detection",  "Phase Detection"),
  param = c("Weight on negative class", "Weight on positive classes", "Weights on classes"),
  values = c("1", "40, 60, 80", "Inv. class freq.: c(1.89, 0.67, 1.90, 0.69)")
) |> 
  dplyr::group_by(strat) |> 
  dplyr::mutate(strat = ifelse(dplyr::row_number() == 1, strat, "")) |> 
  dplyr::ungroup() |> 
  gt::gt() |> 
  gt::tab_style(
    style = list(
...
Class weight tuning grid values.
...
data.frame(
  ...
  gt::cols_label(
    model = "Model",
    youden = "Event detection: weighted Youden index",
    accuracy = "Phase detection: accuracy"
  ) |> 
  gt::tab_options(column_labels.background.color = "#616161") |> 
  gt::tab_style(
...
Score on the test set for both strategies.
...
::: {#fig-preds-both-strategies layout="[45, -10, 45]"}
...
Metrics of the selected model on the test set.

:::

For a complete comparison of our model with the gold standard, we use spatio-temporal parameters that can be computed from the four events happening during a walking cycle: the number of cycles in a session, the cycle lengths and the percentage of stance phase. We compute the mean value for these parameters on each session from the test set and for each foot by computing the gait cycles starting with the right and the left foot. For the number of cycles in each session, we find the exact same number of cycles for our model and the gold standard, which is not surprising since we previously shown that the model detects the correct number of each event in every sessions. For the mean cycle lengths and the mean percentage of stance phase, we use Bland-Altman plots [@bland_altman] to compare the the results (see @fig-bland-altman).

::: {#fig-bland-altman layout="[45, -10, 45]"}

![Comparison of the mean cycle length (seconds).](images/ba_cycle_length.png){#fig-ba-cycle-length width=270}
