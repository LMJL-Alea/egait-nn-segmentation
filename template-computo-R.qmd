...
To choose a final model, we need to determine the best strategy. In order to do so, we compare the two best models with the real events given by the gold standard to see which model gives closer results.
...
In both strategies, the model does not detect only one point corresponding to the event but a window of points containing the event (see @fig-res-pred-events and @fig-res-pred-phases). This means that we need to build a procedure to get precise estimated event times from the model predictions. We can use the same process for both strategies to choose one point either in the windows detected with the event strategy or in the sub-phases predicted with the phase strategy.

::: {layout="[45, -10, 45]"}

![Example of predictions made by the neural network when predicting events.](images/preds-events.png){#fig-res-pred-events width=250}

![Example of predictions made by the boosted trees when predicting phases.](images/preds-phases.png){#fig-res-pred-phases width=250}
...
Then, in each of these clearly separated windows or phases, we keep the point having the highest probability of being in this class according to the model. This point is the time of the estimated event.

We can then compare these estimated events with the ones given by the gold standard to see which of the strategy gives better result and select a final strategy.
