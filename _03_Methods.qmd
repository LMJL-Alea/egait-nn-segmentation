...
This strategy aims at directly predicting the occurence of gait events of interest. However, it inherits by construction of a severe class imbalance  effect, with the *None* class widely over-represented. Indeed, the class that represents all the times that do not belong to an event is clearly larger than the four other ones. It is clearly represented on @fig-event-timeserie where we can see the event times from the GAITRite© mat overlaid on the QTS recorded by the IMU. Each colored point represents a different event and all other times belong to the *None* class.

::: {.aside}
[AST] A table showing the counts of each class could help making the point more quantitative.
:::
...
When the classes are not too imbalanced, such as in Strategy P (phase prediction), we can use the

[inverse class frequency]{.mark}

::: {.aside}
[AST] A reference to a paper using this method could be useful.
:::

weights where the weight on the class $k$ is given by:
...
In this strategy, we aim at predicting occurences of gait events directly. As a result, four of the five classes are the classes of interest while the majority class is not relevant. Furthermore, the majority class contains

[XXX% of the observations]{.mark}

::: {.aside}
[AST] A precise percentage could help making the point more quantitative.
:::

, which represents a case of severe class imbalance. Accuracy is not adapted in this case because a model which always predicts the majority class will be deemed a *good* model in the sense of accuracy, while completely missing the classes of interest. We therefore need to use other metrics that are adapted to this situation.
