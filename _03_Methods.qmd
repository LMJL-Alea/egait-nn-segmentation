...
```{r}
tibble::tibble(
  class = c("Right Heel Strike", "Left Toe Off", "Left Heel Strike", "Right Toe Off", "None"),
...
Strategy E: Count and proportion of observations in each class.
...
[MSI] recall = sensitivity; precision is used when the cost of a False Positive is high but in our case we don't mind classifying points as part of classes of interest even if they belong to the None class. We prefer to put too many observations in the classes of interest and then select high probability points within them.
:::
