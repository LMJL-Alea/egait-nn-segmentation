...
We dedicated the previous @sec-material to presenting the available data and elaborating the input labelled IMU data. This section aims at eliciting the methods that we designed to train, tune and comparing several models with the aim of detecting the key gait events RHS, LTO, LHS and RTO from IMU data exclusively, possibly in a streamlined fashion. @sec-classification-strategies explores two different strategies that we compared to achieve this goal. @sec-feature-space lists the variables that we computed to be included in the feature space for training and tuning the models. @sec-models describes the models themselves. @sec-class-imbalance details how we dealt with the class imbalance problem. @sec-performance-metrics summarizes the performance metrics that we used to tune and compare models. Finally, @sec-data-splitting explains the procedure that we adopted to split the data set for training, tuning and testing the models.
...
```{r}
#| label: tbl-event-freq
#| tbl-cap: "Absolute and relative frequencies of the different gait events in the final labelled IMU data set."
#| tbl-pos: "H"
labelled_imu_data |>
...
@tbl-event-freq reveals that the task of predicting gait events of interest is a severe case of class imbalance, with less than 1\% of timepoints in the classes of interest while more than 95\% of timepoints end up in the negative class. To mitigate this issue, we can frame the problem differently and build models to predict membership of timepoints to gait *phases* instead of gait *events*. In effect, as explained in @sec-gaitrite-data, gait events define four phases within the typical gait cycle: pre-stance (between RHS and LTO), stance (between LTO and LHS), pre-swing (between LHS and RTO) and swing between (between RTO and RHS). The code below augments the labelled IMU data set with the membership to gait phases and produces the corresponding frequency table:

```{r}
#| label: tbl-phase-freq
#| tbl-cap: "Absolute and relative frequencies of the different gait phases in the final labelled IMU data set."
#| tbl-pos: "H"
...
labelled_imu_data <- labelled_imu_data |>
  purrr::map(\(.imu_data) {
    .imu_data |>
      dplyr::mutate(
...
labelled_imu_data |>
  dplyr::bind_rows(.id = "session_id") |>
  dplyr::mutate(session_id = as.numeric(session_id)) |>
...
@tbl-phase-freq shows that the class imbalance issue is much less severe when we aim at predicting gait phases. As illustrated in @fig-imu-data-with-phases, the two phases labelled starting with *Pre-* are shorter than the other two as they represent approximately 10\% of the gait cycle each while stance and swing phases are 40\%-long.

```{r}
#| label: fig-imu-data-with-phases
#| fig-cap: "Filtered IMU data (33rd session) colored by gait phases with key gait events superimposed with colored points as well."
#| fig-pos: "H"
df <- tidyr::pivot_longer(
  labelled_imu_data[[33]],
  cols = -c(time, event_type, phase_type)
)

df |>
  ggplot(aes(x = time, y = value)) +
  geom_point(mapping = aes(color = phase_type), size = 1) +
  geom_line() +
  geom_point(
    data = dplyr::filter(df, event_type != "None"),
    mapping = aes(group = event_type),
    size = 2
  ) +
  facet_wrap(vars(name), ncol = 1, scales = "free") +
  theme_bw() +
  labs(x = "Time (s)", y = "", color = "Phases") +
  theme(legend.position = "top")
```

In the end, we therefore designed two classification strategies:

- **Strategy E**, where **E** stands for [E]{.underline}vents, which aims at predicting gait events of interest directly but suffers from a severe class imbalance issue;
- **Strategy P**, where **P** stands for [P]{.underline}hases, which aims at predicting gait phases hence reducing class imbalance at the cost of requiring post-processing to pinpoint the exact times of occurence of gait events of interest.

## Feature space {#sec-feature-space}

In this work, the objective is to identify which timepoints of unit QTS correspond to the RHS, LTO, LHS and RTO events. For this purpose, we consider the timepoints as statistical units (observations) and we aim at labelling them by means of supervised classification models. We therefore need to define a so-called *feature space* which consists of a data table listing the timepoints by row and collecting a number of features for each of them. A first important feature is the actual label that we want to predict with the trained model. @sec-labelled-imu-data details the elaboration of what
...
```{r}
saveRDS(labelled_imu_data, file = "~/Downloads/labelled_imu_data.rds")
```
...
labelled_imu_data |>
  purrr::map(\(.imu_data) {
    tmp <- .imu_data |>
      dplyr::select(time, w, x, y, z) |>
      dplyr::filter(time < 7) |>
      squat::as_qts() |>
      squat::qts2avvts()
  }) |>
  dplyr::bind_rows(.id = "session_id") |>
  dplyr::mutate(session_id = as.factor(session_id)) |>
  dplyr::filter(session_id %in% 1:20) |>
  tidyr::pivot_longer(cols = -c(time, session_id)) |>
  ggplot(aes(x = time, y = value, color = session_id)) +
  geom_line(alpha = 0.7) +
...
labelled_imu_data |>
  ...
  dplyr::mutate(session_id = as.factor(session_id)) |>
  dplyr::filter(session_id %in% 1:20) |>
  ggplot(aes(x = time, y = magnitude, color = session_id)) +
  geom_line(alpha = 0.7) +
  theme_bw() +
  theme(legend.position = "none")
```
...
## Supervised classification models {#sec-models}
...
## Data splitting and tuning strategy {#sec-data-splitting}
