...
Even if many methods have been implemented to segment gait data and classify its events of phases, we did not find in the literature a method based on hip orientation data represented as unit quaternion time-series which is the particularity of the data type used in our work.
...
[MSI] We don't know if it would lead to better results to first segment strides. But the advantage of our method is its simplicity and maybe its quickest computation time since it's only one model to predict from. I wouldn't have put too many details over this in the paper since we did not compare the two methods.
:::
